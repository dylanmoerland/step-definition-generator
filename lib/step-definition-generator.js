...
const converter = {
	...
	init: function(data) {
		...
		this.items = this.getItems([...new Set(this.normalize(data.content))]);
		this.items = this.resolveTypes(this.items);
		this.items = this.orderByType(this.items);
		this.items = this.withParameters(this.items);
...
const documenter = {
	...
	output: '',
	insert: atom.config.get('feature-to-steps.pendingMethod', ''),
...
export default {
  ...
  activate(state) {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'feature-to-steps:generate': () => this.generate(),
    }));
  },
